...
SEALED_SECRET_INSTANCE=$(oc get deployment -n "${NAMESPACE}" -l app.kubernetes.io/instance=sealed-secrets -o json | jq -r '.items[] | .metadata.name' | head -1)
SEALED_SECRET_CREATED_BY=$(oc get deployment -n "${NAMESPACE}" -l app.kubernetes.io/instance=sealed-secrets -o json | jq -r '.items[] | .metadata.labels["created-by"] // empty')

if [[ "${SEALED_SECRET_CREATED_BY}" == "${CREATED_BY}" ]]; then
  ...
  echo '{"exists": "false"}'
  exit 0
elif [[ -n "${SEALED_SECRET_INSTANCE}" ]]; then
  echo '{"exists": "true"}'
  exit 0
else
  echo '{"exists": "false"}'
